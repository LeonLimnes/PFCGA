...
vec3 randomInitialVelocity() {
    float velocity = mix(0.1, 10.0, texelFetch(RandomTex, 2 * gl_VertexID, 0).r );
    return EmitterBasis * vec3(0, velocity, 0);
}

vec3 randomInitialPosition() {
    float offsetX = mix(-50.0, 50.0, texelFetch(RandomTex, 2 * gl_VertexID + 1, 0).r);
	float offsetZ = mix(-50.0, 50.0, texelFetch(RandomTex, 2 * gl_VertexID + 2, 0).r);
