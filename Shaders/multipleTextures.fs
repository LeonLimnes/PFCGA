...
vec3 calculateDirectionalLight(Light light, vec3 direction){
	vec3 colorTextura1 = vec3(texture(texture1, our_uv));
	vec3 colorTextura2 = vec3(texture(texture2, our_uv));
	vec3 colorTextura = mix(colorTextura1, colorTextura2, 0.3);

	// Ambient
    vec3 ambient  = light.ambient * colorTextura;
  	
    // Diffuse 
    vec3 normal = normalize(our_normal);
    vec3 lightDir = normalize(-direction);
    float diff = max(dot(normal, lightDir), 0.0);
    vec3 diffuse  = light.diffuse * (diff * colorTextura);
    ...
    vec3 specular = light.specular * (spec * colorTextura);  
